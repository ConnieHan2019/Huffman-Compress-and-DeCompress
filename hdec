...
public class hdec{
    ...
    static int line1;
    static int line2;
    public static int frequencyCounter;
	...
	public static void main(String[] args) throws IOException
	{
		try
		{
			String sourcepath ="E:/Project 1/Test Cases/test1 - single file/0.txt.huf";
	...
	public static void reformMui_de(String[]nameOfFile, int[]lenOfmultiStream,File file) throws IOException {
		...
		for (int i = 0; i < numberOfFile; i++){
			...
			name = nameOfFile[i];
			BufferedOutputStream bwrite = new BufferedOutputStream(new FileOutputStream(nameOfFile[i]));
		    //while (b1.read() != '\n') ;
		   // while (b1.read() != '\n') ;
		    int y = 0;
           for(int j = 0; j< freq;j++){
			   ...
			   if(len >= 512){
				   ...
				   buildCode.delete(0,writeback(bwrite,buildCode,lenOfmultiStream[i],frequencyCounter));
